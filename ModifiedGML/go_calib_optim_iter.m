...
%removing images with no useful information => reducing JJ3 size (JJ2_inv
%as well)
if (exist('active_images','var'))
    num_images=size(active_images,2);

    max_images_default=600;
    max_images=input(sprintf('How many frames ? ([]=%g / maximum:%g )',max_images_default,num_images));

    if (isempty(max_images))
        max_images=max_images_default;
    end

    centros=nan(num_images,2);

    for i=1:num_images
        if (active_images(i)==1)
            eval(sprintf('xAtual=x_%g;',i));
            centros(i,:)=sum(xAtual,2)/size(xAtual,2);
        end
    end
    
    if (sum(~isnan(centros(:,1)))>max_images)
        
        active_images=zeros(size(active_images));
        
        [idx C]=kmeans(centros,max_images);
        for i=1:max_images
            indice=find(idx==i,1,'first');
            active_images(indice)=1;
        end
    end
end
...
if ~exist('MaxIter','var'),
    MaxIter = 60; % Maximum number of iterations in the gradient descent
end;
